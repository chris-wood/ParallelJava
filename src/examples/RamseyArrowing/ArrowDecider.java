...
public class ArrowDecider 
{
	...
	public boolean arrowsTriangles(int[][] F)
	{
		int colors = 2;
		...
		BigInteger upper = BigInteger.ONE; // 1
		BigInteger exponent = BigInteger.ONE.add(BigInteger.ONE); // 2
		for (int power = 0; power < edgeCount; power++)
		{
			upper = exponent.multiply(upper);
		}
	...
	public static void main(String[] args)
	{
		if (args.length != 1) 
		{
			System.err.println("usage: java ArrowDecider n");
			System.exit(-1);
		}
		...
		ArrowDecider decider = new ArrowDecider();
		System.out.println(decider.arrowsTriangles(F));
	}
}